...
const RecieptName = () => {
  ...
  const fetchEmails = useCallback(async () => {
    ...
    try {
      ...
      setLoading(false); // Ensures loading is false even if fetch fails
    }
  }, []); // Empty array means this function is created only once

  // 2. Use the stable function in useEffect
  useEffect(() => {
    fetchEmails();
  }, [fetchEmails]); // Now safe to include as a dependency

  // 3. deleteEmail can stay as is, but it will use the stable fetchEmails()
