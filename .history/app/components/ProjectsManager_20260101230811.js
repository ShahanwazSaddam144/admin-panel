...
const ProjectsManager = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      // 1. Create the new state object with the updated field
      const updatedData = { ...prev, [name]: value };

      // 2. If the user is changing a date, calculate days immediately
    ...
    <section className="mt-10 px-4 md:px-0">
      ...
      <form onSubmit={handleSubmit} className="w-full p-6 space-y-6 rounded-xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          <label className="md:w-40 text-sm font-medium text-gray-700">Project Name</label>
          <input
            type="text"
            name="ProjectName"
        ...
        <div className="flex flex-col md:flex-row md:items-start gap-3">
          <label className="md:w-40 text-sm font-medium text-gray-700 md:pt-2">Project Detail</label>
          <textarea
            name="ProjectDetail"
        ...
        {/* Project Link */}
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          <label className="md:w-40 text-sm font-medium text-gray-700">Project Link</label>
          <input
            type="text"
            name="ProjectLink"
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex flex-col md:flex-row md:items-center gap-3 w-full">
            <label className="md:w-40 text-sm font-medium text-gray-700">Start Date</label>
            <input
              type="date"
              name="StartDate"
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 w-full">
            <label className="md:w-40 text-sm font-medium text-gray-700">End Date</label>
            <input
              type="date"
              name="EndDate"
        ...
        {/* Days Consumed */}
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          <label className="md:w-40 text-sm font-medium text-gray-700">Days Consumed</label>
          <input
            type="number"
            name="DaysConsumed"
      ...
      {/* Popup Toast */}
      {popup.show && (
        <div className={`fixed bottom-8 right-8 px-6 py-4 rounded-lg shadow-lg text-white ${popup.type === "success" ? "bg-green-500" : "bg-red-500"}`}>
          {popup.message}
        </div>
      )}
    </section>
