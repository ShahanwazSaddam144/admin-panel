...
router.delete("/emails/:id", protect, async (req, res) => {
  try {
    const userId = req.user.id; // Or req.user.uid, depending on your token payload
    const emailId = req.params.id;

    // Use findOneAndDelete to ensure the email belongs to the user
    const deletedEmail = await Email.findOneAndDelete({
      _id: emailId,
      user: userId, // This ensures ONLY the owner can delete it
    });

    if (!deletedEmail) {
      // If not found, it's either the wrong ID or the wrong user
