...
router.get("/language",protect, async (req, res) => {
  try {
    To filter the results by the user ID stored in your req.user object, you just need to pass the ID as a query filter to the find() method.

Assuming your decoded token contains a property like id or _id, here is how you would update your route handler:

JavaScript

router.get('/my-charts', protect, async (req, res) => {
  try {
    // req.user was populated by your 'protect' middleware
    const userId = req.user.id; 

    // Filter by the user field in your schema
