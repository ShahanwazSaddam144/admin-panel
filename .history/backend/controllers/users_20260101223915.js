...
router.post("/signup", async (req, res) => {
  ...
  if (!name || !email || !pass) return res.status(400).json({ message: "Missing credentials" });
...
router.post("/login", async (req, res) => {
  ...
  if (!email || !pass) return res.status(400).json({ message: "Missing credentials" });

  try {
    const user = await User.findOne({ email, pass });
    if (!user) return res.status(401).json({ message: "Invalid credentials" });

    const token = jwt.sign({uid:user._id, email: user.email, name: user.name }, JWT_SECRET, { expiresIn: "1h" });
