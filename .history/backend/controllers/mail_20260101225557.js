...
router.post("/send-email", protect, async (req, res) => {
  ...
  try {
    ...
    // Save in Database
    const emailRecord = new Email({ to, subject, text });
...
router.get("/emails", protect, async (req, res) => {
  try {
    // Filter by the user ID from the decoded token
    const userId = req.user.id;

    // Only fetch emails belonging to this specific user
    const emails = await Email.find({ user: userId }).sort({ createdAt: -1 });

    res.json({
      success: true,
      count: emails.length, // Helpful for the frontend to know the list size
      emails,
    });
  } catch (err) {
