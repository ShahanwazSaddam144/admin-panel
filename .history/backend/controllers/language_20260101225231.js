...
router.delete("/language/:id", protect, async (req, res) => {
  try {
    // 1. Get the chart ID from params and User ID from the middleware
    const chartId = req.params.id;
    const userId = req.user.id;

    // 2. Find by BOTH ID and User ID to ensure ownership
    const deletedChart = await Charts.findOneAndDelete({
      _id: chartId,
      user: userId,
    });

    // 3. If no document was found, it either doesn't exist OR it doesn't belong to the user
    if (!deletedChart) {
